...
export default function ArtifactPage() {
  ...
  const pieceAuthor = piece?.author;
  const authorName = pieceAuthor?.name || 'Unknown Author';
  const authorID = pieceAuthor?.id || 'Unknown';
  const community = piece?.community;
  const backRoute = community ? ROUTES.VILLAGE.HOME(community.id) : ROUTES.EXPLORE;
          ...
          <header className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4 border-y border-theme-outline/20 py-4">
              <div className="flex -space-x-2 mr-2">
                <div key={authorID} className="w-10 h-10 rounded-full border-2 border-theme-parchment bg-theme-clay flex items-center justify-center text-xs font-bold text-white shadow-sm" title={authorName}>
